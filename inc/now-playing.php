...
Note - ideally I'd be using Spotify API for this, but I
wanted to play with the last.fm API (also, no OAuth!)
--------------*/
...
function getLastFm() {
    ...
    if ( empty( $artwork ) ) {  // Check if album artwork exists on last.fm, else use our own fallback image
        $artwork = get_template_directory_uri() . '/img/artwork-fallback.png';
    }
    ...
    $trackInfo = [
        ...
        'albumArt'   => $artwork,
        'status'     => $status // Display 'Status' from the above cURL request.
    ];
